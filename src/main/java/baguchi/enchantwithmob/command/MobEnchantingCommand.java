...
public class MobEnchantingCommand {
    ...
    private static int setMobEnchants(CommandSourceStack commandStack, Entity entity, Holder.Reference<MobEnchant> holder, int level) {
		...
		if (entity != null) {
			if (entity instanceof LivingEntity) {
				if (mobEnchant != null) {
					if (level > mobEnchant.value().getMaxLevel()) {
						...
						commandStack.sendSuccess(() -> Component.translatable("commands.enchantwithmob.mob_enchanting.set_enchant", entity.getDisplayName(), commandStack.registryAccess().lookupOrThrow(ModRegistries.MOB_ENCHANT).getKey(mobEnchant.value()).toString()), true);
						return 1;
					}
				} else {
